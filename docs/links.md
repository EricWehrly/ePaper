* Device: [Waveshare 4-inch e-Paper HAT+ (E)](https://www.waveshare.com/wiki/4inch_e-Paper_HAT%2B_%28E%29_Manual)
* Example code we are starting from: [`epd_4in0e_test.py`](https://github.com/waveshareteam/e-Paper/blob/master/RaspberryPi_JetsonNano/python/examples/epd_4in0e_test.py)
* Library reference: [`waveshare_epd/epd4in0e.py`](https://github.com/waveshareteam/e-Paper/tree/master/RaspberryPi_JetsonNano/python/lib/waveshare_epd)

## Google Photos API Research

### Official Documentation
* [Google Photos APIs Overview](https://developers.google.com/photos/overview/about)
* [Picker API Documentation](https://developers.google.com/photos/picker/guides/get-started-picker)
* [Picker API Reference](https://developers.google.com/photos/picker/reference/rest)
* [API Changes (March 2025)](https://developers.google.com/photos/support/updates)
* [Authorization & Scopes](https://developers.google.com/photos/overview/authorization)
* [Rate Limits & Quotas](https://developers.google.com/photos/overview/api-limits-quotas)

### Setup Resources  
* [Google Cloud Console](https://console.cloud.google.com/)
* [Complete Google Cloud Setup Guide](./google_cloud_setup.md) ⭐ **Start Here**
* [API Configuration Guide](https://developers.google.com/photos/overview/configure-your-app)
* [OAuth 2.0 Setup](https://developers.google.com/identity/protocols/oauth2)

### Code Examples
* [Official Picker API Samples](https://developers.google.com/photos/picker/samples)
* [Python OAuth Examples](https://developers.google.com/identity/protocols/oauth2/web-server#python)

### Project Documentation
* [Feasibility Study](./google_photos_feasibility_study.md)
* [Implementation Roadmap](./google_photos_roadmap.md)
* [Phase 1 Implementation Guide](./google_photos_implementation.md)